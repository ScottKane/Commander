...
public abstract class SteamCmdCommand
{
    ...
    protected abstract ValueTask ParseAsync(IConsole console, SteamCMDConPTY connection, string? line);

    private async ValueTask ParseAsyncCore(IConsole console, SteamCMDConPTY connection, string? line)
    {
    ...
    {
        var connection = new SteamCMDConPTY
        {
            Arguments = string.Join(' ', Command.Append("+quit")),
            WorkingDirectory = SteamCmdPath
        };
        
        // connection.TitleReceived += (sender, data) => { };

        connection.OutputDataReceived += async (_, data) =>
        {
            await ParseAsyncCore(console, connection, data);
            await console.Output.WriteLineAsync(data);
        };

        connection.Exited += (_, exitCode) => _process.TrySetResult((uint)exitCode);;

        var processInfo = connection.Start();
